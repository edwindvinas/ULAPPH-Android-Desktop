...
if (!ULAPPH_TOOLS_DESK) {
  var ULAPPH_TOOLS_DESK = {};
}

ULAPPH_TOOLS_DESK.Util = {
...
ULAPPH_TOOLS_DESK.Dialog = {
  ...
  show: function(id_or_html) {
    ...
    this.Overlay.show();
    this.setPosition();
    ULAPPH_TOOLS_DESK.Element.addClassName(this.htmlElement(), 'dialog-open');
  ...
  close: function() {
		var change = ULAPPH_TOOLS_DESK.needsConfirm;
		if(change){
    ...
    ULAPPH_TOOLS_DESK.Element.removeClassName(this.htmlElement(), 'dialog-open');
    this.Overlay.hide();
    ULAPPH_TOOLS_DESK.onClose();
  },
  ...
  (ULAPPH_TOOLS_DESK.Util.isIE() && (ULAPPH_TOOLS_DESK.Util.isIE(function(v){return v < 7}) || (ULAPPH_TOOLS_DESK.Util.isIE(function(v){return v >= 7}) && ULAPPH_TOOLS_DESK.Util.isQuirksMode()))) ? "\
    #ulapph-overlay,\
    #ulapph-dialog {\
      position: absolute;\
    }\
  ...
  element: function() {
    if (!document.getElementById(this.id)){
      ...
      dummy.innerHTML = '<div id="'+this.id+'" class="ulapph-component" style="display:none;">' +
        '<a href="#close" onclick="ULAPPH_TOOLS_DESK.Dialog.close(); return false;" id="'+this.id+'-close" title="Close Dialog"><span style="display: none;">Close Dialog</span></a>' +
  ...
  setPosition: function() {
    var dialogDimensions = ULAPPH_TOOLS_DESK.Element.getDimensions(this.element());
    var pageDimensions = ULAPPH_TOOLS_DESK.Page.getDimensions();
...
ULAPPH_TOOLS_DESK.Dialog.Overlay = {
  ...
  element: function() {
    if (!document.getElementById(this.id)) {
      var dummy = document.createElement('div');
      dummy.innerHTML = '<div id="'+this.id+'" class="ulapph-component" onclick="ULAPPH_TOOLS_DESK.Dialog.close(); return false;" style="display:none;"></div>';
      /* this is the safe way to do insertion in IE when the page has yet to be loaded */
      document.body.insertBefore(dummy.firstChild, document.body.firstChild);
    }
    return document.getElementById(this.id);
...
ULAPPH_TOOLS_DESK.needsConfirm = false;
ULAPPH_TOOLS_DESK.onClose = function() {};

// Load CSS and images
ULAPPH_TOOLS_DESK.Util.includeCss(ULAPPH_TOOLS_DESK.Util.render(ULAPPH_TOOLS_DESK.Dialog.css_template, {background_image_url: 'http://lh4.ggpht.com/v5vgvBUHeXAzLcx-xLhVWRAo6IScq0eSPkQhGQtIu6tsvTc-ULjE0NS4E6oKXosntn5mjr9ZBaQFuBcBuUyDI7KVdFrvzmIlJA'}));

ULAPPH_TOOLS_DESK.PopIn = {
  show: function() {
