...
ULAPPH_SLIDES.Dialog = {
  ...
  show: function(id_or_html) {
    ...
    this.Overlay.show();
    this.setPosition();
    ULAPPH_SLIDES.Element.addClassName(this.htmlElement(), 'dialog-open');
  ...
  close: function() {
		var change = ULAPPH_SLIDES.needsConfirm;
		if(change){
    ...
    ULAPPH_SLIDES.Element.removeClassName(this.htmlElement(), 'dialog-open');
    this.Overlay.hide();
    ULAPPH_SLIDES.onClose();
  },
  ...
  (ULAPPH_SLIDES.Util.isIE() && (ULAPPH_SLIDES.Util.isIE(function(v){return v < 7}) || (ULAPPH_SLIDES.Util.isIE(function(v){return v >= 7}) && ULAPPH_SLIDES.Util.isQuirksMode()))) ? "\
    #ulapph-overlay,\
    #ulapph-dialog {\
      position: absolute;\
    }\
  ...
  element: function() {
    if (!document.getElementById(this.id)){
      ...
      dummy.innerHTML = '<div id="'+this.id+'" class="ulapph-component" style="display:none;">' +
        '<a href="#close" onclick="ULAPPH_SLIDES.Dialog.close(); return false;" id="'+this.id+'-close" title="Close Dialog"><span style="display: none;">Close Dialog</span></a>' +
  ...
  setPosition: function() {
    var dialogDimensions = ULAPPH_SLIDES.Element.getDimensions(this.element());
    var pageDimensions = ULAPPH_SLIDES.Page.getDimensions();
...
ULAPPH_SLIDES.Dialog.Overlay = {
  ...
  element: function() {
    if (!document.getElementById(this.id)) {
      var dummy = document.createElement('div');
      dummy.innerHTML = '<div id="'+this.id+'" class="ulapph-component" onclick="ULAPPH_SLIDES.Dialog.close(); return false;" style="display:none;"></div>';
      /* this is the safe way to do insertion in IE when the page has yet to be loaded */
      document.body.insertBefore(dummy.firstChild, document.body.firstChild);
    }
    return document.getElementById(this.id);
...
ULAPPH_SLIDES.needsConfirm = false;
ULAPPH_SLIDES.onClose = function() {};

// Load CSS and images
ULAPPH_SLIDES.Util.includeCss(ULAPPH_SLIDES.Util.render(ULAPPH_SLIDES.Dialog.css_template, {background_image_url: 'https://lh4.ggpht.com/v5vgvBUHeXAzLcx-xLhVWRAo6IScq0eSPkQhGQtIu6tsvTc-ULjE0NS4E6oKXosntn5mjr9ZBaQFuBcBuUyDI7KVdFrvzmIlJA'}));
