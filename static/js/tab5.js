...
ULAPPH_QR.Dialog = {
  ...
  show: function(id_or_html) {
    ...
    this.Overlay.show();
    this.setPosition();
    ULAPPH_QR.Element.addClassName(this.htmlElement(), 'dialog-open');
  ...
  close: function() {
		var change = ULAPPH_QR.needsConfirm;
		if(change){
    ...
    ULAPPH_QR.Element.removeClassName(this.htmlElement(), 'dialog-open');
    this.Overlay.hide();
    ULAPPH_QR.onClose();
  },
  ...
  (ULAPPH_QR.Util.isIE() && (ULAPPH_QR.Util.isIE(function(v){return v < 7}) || (ULAPPH_QR.Util.isIE(function(v){return v >= 7}) && ULAPPH_QR.Util.isQuirksMode()))) ? "\
    #ulapph-overlay,\
    #ulapph-dialog {\
      position: absolute;\
    }\
  ...
  element: function() {
    if (!document.getElementById(this.id)){
      ...
      dummy.innerHTML = '<div id="'+this.id+'" class="ulapph-component" style="display:none;">' +
        '<a href="#close" onclick="ULAPPH_QR.Dialog.close(); return false;" id="'+this.id+'-close" title="Close Dialog"><span style="display: none;">Close Dialog</span></a>' +
  ...
  setPosition: function() {
    var dialogDimensions = ULAPPH_QR.Element.getDimensions(this.element());
    var pageDimensions = ULAPPH_QR.Page.getDimensions();
...
ULAPPH_QR.Dialog.Overlay = {
  ...
  element: function() {
    if (!document.getElementById(this.id)) {
      var dummy = document.createElement('div');
      dummy.innerHTML = '<div id="'+this.id+'" class="ulapph-component" onclick="ULAPPH_QR.Dialog.close(); return false;" style="display:none;"></div>';
      /* this is the safe way to do insertion in IE when the page has yet to be loaded */
      document.body.insertBefore(dummy.firstChild, document.body.firstChild);
    }
    return document.getElementById(this.id);
...
ULAPPH_QR.needsConfirm = false;
ULAPPH_QR.onClose = function() {};

// Load CSS and images
ULAPPH_QR.Util.includeCss(ULAPPH_QR.Util.render(ULAPPH_QR.Dialog.css_template, {background_image_url: 'http://lh4.ggpht.com/v5vgvBUHeXAzLcx-xLhVWRAo6IScq0eSPkQhGQtIu6tsvTc-ULjE0NS4E6oKXosntn5mjr9ZBaQFuBcBuUyDI7KVdFrvzmIlJA'}));

ULAPPH_QR.PopIn = {
  show: function(qrUrl) {
	//alert(qrUrl);
	var str = encodeURIComponent(qrUrl);
	var res = str.replace(/%26/g, '@888@');
	//alert(res);
	var url = "/social?SO_FUNC=QRGEN&SID=QRGEN&QRDAT=" + res;
    ULAPPH_QR.Dialog.show("<iframe src=\"" + url + "\" frameborder=\"0\" scrolling=\"yes\" allowtransparency=\"true\" width=\"350px\" height=\"350\"></iframe>");
  }
}
